                ...
                {item.hasSubmenu ? (
                  <>
                    <SidebarMenuButton asChild>
                      <button
                        onClick={() => toggleSubmenu(item.label)}
                        className="flex items-center justify-between w-full gap-3 rounded-lg hover:text-primary transition-all"
                      >
                        <div className="flex items-center gap-3">
                          {item.icon}
                          <span>{item.label}</span>
                        </div>
                        <span className={`transition-transform ${openSubmenu === item.label ? "rotate-180" : "rotate-0"}`}>
                          {openSubmenu === item.label ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                        </span>
                      </button>
                    </SidebarMenuButton>
                    {openSubmenu === item.label && (
                      <div className="ml-4 mt-2 space-y-2">
                        <SidebarMenu>
                          {item.subItems?.map((subItem) => (
                            <SidebarMenuItem key={subItem.label}>
                              <SidebarMenuButton asChild>
                                <Link href={subItem.href || "#"} className="flex items-center gap-3 rounded-lg py-1 hover:text-primary">
                                  {subItem.label}
                                </Link>
                              </SidebarMenuButton>
                            </SidebarMenuItem>
                  ...
                  <SidebarMenuButton asChild>
                    <Link href={item.href || "#"} className="flex items-center gap-3 rounded-lg hover:text-primary">
                      {item.icon}
                      <span>{item.label}</span>
                    </Link>
                  </SidebarMenuButton>
